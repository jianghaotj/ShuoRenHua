...
根据用户选择的文学风格、类型、叙述距离与读者安排作品，不强加帖子口气。这里的场景、视角与声音方法服从作品需要，不是所有中文任务的默认写法。

虚构不强制结论先行、通用句长阈值、单句单信息或术语式定义；诗歌、剧本和人物对白可以省略、跳切与停顿。事实支持和现实来源的规则统一见 [reality.md](reality.md)，用户指定改写范围见 [editing.md](editing.md)。
...
用户明确要求按自己的真实经历写时，按 F01/F02 保护用户材料。允许文学化改写时沿已授权范围创作；只有会改变核心事实且授权不明的部分才询问，不为已经明确的创作空间重复确认。

## 现实部分按需核准

纯架空世界只检查内部规则。

故事依赖真实城市、历史、职业、药物、法律或技术时，按 F01/F04 核对会改变现实判断的重要内容；有意偏离现实的设定应让读者能分辨。纯虚构场景中的房间摆设、天气和路人可以创造。
...
故事已经让读者感到什么，可以让它停在后果里。叙述者的总结若符合声音并带来新理解，也可以保留，不为了普遍规则删掉作品需要的收束。

需要针对任务的对照例时，按需读取 [examples/narrative.md](examples/narrative.md) 中一两组，不加载整个示例库。
